...
import com.fank243.cloud.auth.shiro.filter.JWTShiroFilter;
import com.fank243.cloud.auth.shiro.filter.PermShiroFilter;
import com.fank243.cloud.auth.shiro.filter.RoleShiroFilter;
import com.fank243.cloud.auth.shiro.realm.JWTShiroRealm;
import com.fank243.cloud.auth.shiro.service.impl.ShiroServiceImpl;
...
public class ShiroConfiguration {
     ...
     * 缓存管理器
     * 
     * 1.在调用{@link JWTShiroRealm#doGetAuthorizationInfo(PrincipalCollection)}方法时会将当前登录用户实体对象{@link com.fank243.cloud.auth.shiro.SysShiroUser}
     * 缓存到redis中，因此SysShiroUser对象必须要实现Serializable接口，否则shiro会抛出异常
     * 
     * 2.在调用{@link com.fank243.cloud.auth.web.controller.LoginController#logout()}方法退出成功后会从redis中移除实体对象
     */
    @Bean
